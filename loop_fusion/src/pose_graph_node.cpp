...
LoopFusionRunner::LoopFusionRunner(const rclcpp::NodeOptions & options)
{
    ...
    std::string configRoot = configPath.substr(0, configPath.find_last_of('/'));
    std::string repoRoot = configRoot.substr(0, configRoot.find_last_of('/'));

    string vocabulary_file = repoRoot + "/support_files/brief_k10L6.bin";
    cout << "vocabulary_file " << vocabulary_file << endl;
    posegraph.loadVocabulary(vocabulary_file);

    BRIEF_PATTERN_FILE = repoRoot + "/support_files/brief_pattern.yml";
